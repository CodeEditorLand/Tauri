...
fn main() {
	...
	// current data
	let current_data_buffer =
		BufReader::new(File::open(utils::target_dir().join("bench.json")).expect("Unable to read current data file"));
	let current_data: utils::BenchResult =
		serde_json::from_reader(current_data_buffer).expect("Unable to read current data buffer");

	// all data's
	let all_data_buffer = BufReader::new(File::open(tauri_data).expect("Unable to read all data file"));
	...
	utils::write_json(
		tauri_recent.to_str().expect("Something wrong with tauri_recent"),
