...
fn copy_binaries(binaries:ResourcePaths, target_triple:&str, path:&Path, package_name:Option<&String>) -> Result<()> {
	for src in binaries {
		let src = src?;
		...
		if package_name == Some(&file_name) {
			return Err(anyhow::anyhow!(
				"Cannot define a sidecar with the same name as the Cargo package name `{}`. Please change the sidecar \
				 name in the filesystem and the Tauri configuration.",
				file_name
			));
		}
...
#[cfg(unix)]
fn symlink_dir(src:&Path, dst:&Path) -> std::io::Result<()> { std::os::unix::fs::symlink(src, dst) }

/// Makes a symbolic link to a directory.
#[cfg(windows)]
fn symlink_dir(src:&Path, dst:&Path) -> std::io::Result<()> { std::os::windows::fs::symlink_dir(src, dst) }

/// Makes a symbolic link to a file.
#[cfg(unix)]
fn symlink_file(src:&Path, dst:&Path) -> std::io::Result<()> { std::os::unix::fs::symlink(src, dst) }

/// Makes a symbolic link to a file.
#[cfg(windows)]
fn symlink_file(src:&Path, dst:&Path) -> std::io::Result<()> { std::os::windows::fs::symlink_file(src, dst) }
...
fn copy_frameworks(dest_dir:&Path, frameworks:&[String]) -> Result<()> {
	...
	for framework in frameworks.iter() {
		...
		if copy_framework_from(&PathBuf::from("/Library/Frameworks/"), framework, dest_dir)?
			|| copy_framework_from(&PathBuf::from("/Network/Library/Frameworks/"), framework, dest_dir)?
		{
			continue;
		}
	}
...
impl WindowsAttributes {
	...
	#[must_use]
	pub fn new_without_app_manifest() -> Self { Self { app_manifest:None, window_icon_path:Default::default() } }
...
pub fn is_dev() -> bool {
	env::var("DEP_TAURI_DEV").expect("missing `cargo:dev` instruction, please update tauri to latest") == "true"
}
...
/// platform, so **DO NOT** use a [conditional compilation] check that prevents
/// it from running on any of your targets.
///
...
/// variables that are used when generating code and embedding assets.
///
/// # Panics
///
...
pub fn build() {
	if let Err(error) = try_build(Attributes::default()) {
		...
		if error.starts_with("unknown field") {
			print!(
				"found an unknown configuration field. This usually means that you are using a CLI version that is \
				 newer than `tauri-build` and is incompatible. "
			);
...
/// Same as [`build()`], but takes an extra configuration argument, and does not
/// panic.
...
pub fn try_build(attributes:Attributes) -> Result<()> {
	...
	let (mut config, config_paths) = tauri_utils::config::parse::read_from(target, &env::current_dir().unwrap())?;
	...
	match resources {
		BundleResources::List(res) => copy_resources(ResourcePaths::new(res.as_slice(), true), target_dir)?,
	...
	if target_triple.contains("darwin") {
		if let Some(frameworks) = &config.bundle.macos.frameworks {
			if !frameworks.is_empty() {
				...
				// copy frameworks to the root `target` folder (instead of `target/debug` for
				// instance) because the rpath is set to `@executable_path/../Frameworks`.
	...
	if target_triple.contains("windows") {
		...
		let company_name = config
			.bundle
			.publisher
			.unwrap_or_else(|| config.identifier.split('.').nth(1).unwrap_or(&config.identifier).to_string());
