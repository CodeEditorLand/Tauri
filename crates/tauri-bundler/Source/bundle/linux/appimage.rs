...
pub fn bundle_project(settings: &Settings) -> crate::Result<Vec<PathBuf>> {
	...
	if settings.deep_link_protocols().is_some() && !app_dir_usr_bin.join("xdg-open").exists() {
		fs::copy("/usr/bin/xdg-mime", app_dir_usr_bin.join("xdg-mime")).context("xdg-mime binary not found")?;
	}

	// we also check if the user may have provided their own copy already
	if settings.appimage().bundle_xdg_open && !app_dir_usr_bin.join("xdg-open").exists() {
		fs::copy("/usr/bin/xdg-open", app_dir_usr_bin.join("xdg-open")).context("xdg-open binary not found")?;
	}
	...
	for file in [
		...
		for source in search_dirs.map(PathBuf::from) {
			...
			if source.exists() {
				fs_utils::copy_file(&source, &app_dir_path.join(source.strip_prefix("/").unwrap()))?;
			}
		}
	}

	fs::copy(tools_path.join(format!("AppRun-{tools_arch}")), app_dir_path.join("AppRun"))?;
	fs::copy(
		app_dir_path.join(larger_icon_path),
		app_dir_path.join(format!("{product_name}.png")),
	)?;
	std::os::unix::fs::symlink(app_dir_path.join(format!("{product_name}.png")), app_dir_path.join(".DirIcon"))?;
	...
	if settings.log_level() == log::Level::Error {
		...
		if !cmd.output()?.status.success() {
			return Err(crate::Error::GenericError("failed to run linuxdeploy".to_string()));
		}
	} else {
		cmd.output_ok()?;
	}
