...
use crate::{Settings, bundle::settings::Arch, utils::fs_utils};
use anyhow::Context;
...
pub fn bundle_project(settings: &Settings) -> crate::Result<Vec<PathBuf>> {
	...
	fs_utils::copy_custom_files(&settings.deb().files, &data_dir).with_context(|| "Failed to copy custom files")?;
...
fn generate_control_file(settings: &Settings, arch: &str, control_dir: &Path, data_dir: &Path) -> crate::Result<()> {
	...
	let dependencies = settings.deb().recommends.as_ref().cloned().unwrap_or_default();
	if !dependencies.is_empty() {
