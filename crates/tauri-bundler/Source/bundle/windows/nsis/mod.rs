...
fn try_add_numeric_build_number(version_str:&str) -> anyhow::Result<String> {
	...
	if !version.build.is_empty() {
		...
		if build.is_ok() {
			...
			log::warn!(
				"Unable to parse version build metadata. Numeric value expected, received: `{}`. This will be \
				 replaced with `0` in `VIProductVersion` because Windows requires this field to be numeric.",
				version.build
			);
		}
	}
...
fn build_nsis_app_installer(
	...
	data.insert("homepage", to_json(settings.homepage_url().unwrap_or_default()));
	...
	data.insert("version_with_build", to_json(try_add_numeric_build_number(version)?));
	...
	if let Some(nsis) = nsis {
		if let Some(installer_icon) = &nsis.installer_icon {
			data.insert("installer_icon", to_json(dunce::canonicalize(installer_icon)?));
		}
		...
		if let Some(sidebar_image) = &nsis.sidebar_image {
			data.insert("sidebar_image", to_json(dunce::canonicalize(sidebar_image)?));
		}
	...
	let compression = settings.windows().nsis.as_ref().map(|n| n.compression).unwrap_or_default();
	data.insert(
		"compression",
	...
	data.insert(
		...
		to_json(
			nsis.map(|nsis| nsis.display_language_selector && languages.len() > 1)
				.unwrap_or(false),
		),
	);
	...
	write_utf8_with_bom(output_path.join("FileAssociation.nsh"), include_bytes!("./FileAssociation.nsh"))?;
	...
	let nsis_installer_path = settings.project_out_directory().to_path_buf().join(format!(
		"bundle/{}/{}.exe",
		if updater { NSIS_UPDATER_OUTPUT_FOLDER_NAME } else { NSIS_OUTPUT_FOLDER_NAME },
		package_base_name
	));
	...
	if settings.can_sign() {
		...
		#[cfg(not(target_os = "windows"))]
		log::warn!(
			"Signing, by default, is only supported on Windows hosts, but you can specify a custom signing command in \
			 `bundler > windows > sign_command`, for now, skipping signing the installer..."
		);
	}
...
fn handlebars_or(
	...
	out.write(&if param1.is_empty() { param2.render() } else { param1 })?;
	Ok(())
}
...
fn generate_resource_data(settings:&Settings) -> crate::Result<ResourcesMap> {
	...
	if settings.target().ends_with("-gnu") {
		let loader_path = dunce::simplified(&settings.project_out_directory().join("WebView2Loader.dll")).to_path_buf();
		if loader_path.exists() {
			added_resources.push(loader_path.clone());
			resources.insert(loader_path, (PathBuf::new(), PathBuf::from("WebView2Loader.dll")));
		}
	}
	...
	for resource in settings.resource_files().iter() {
		...
		resources.insert(
			resource_path,
			(
				target_path.parent().expect("Couldn't get parent of target path").to_path_buf(),
