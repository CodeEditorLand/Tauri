...
pub fn migrate(tauri_dir: &Path) -> Result<MigratedConfig> {
	if let Ok((mut config, config_path)) = tauri_utils_v1::config::parse::parse_value(tauri_dir.join("tauri.conf.json"))
	{
	...
	if let Some(config) = config.as_object_mut() {
		...
		if let Some(tauri_config) = config.get_mut("tauri").and_then(|c| c.as_object_mut()) {
			...
			let dangerouse_use_http = tauri_config
				...
				.and_then(|w| w.get("dangerousUseHttpScheme").or_else(|| w.get("dangerous-use-http-scheme")))
				.and_then(|v| v.as_bool())
				.unwrap_or_default();

			if let Some(windows) = tauri_config.get_mut("windows").and_then(|w| w.as_array_mut()) {
			...
			// security
			if let Some(security) = tauri_config.get_mut("security").and_then(|c| c.as_object_mut()) {
	...
	if let Some(mut package_config) = config.remove("package") {
		if let Some(package_config) = package_config.as_object_mut() {
			if let Some((product_name, key)) = package_config
				.remove("productName")
				.map(|v| (v, "productName"))
				.or_else(|| package_config.remove("product-name").map(|v| (v, "product-name")))
			{
...
fn process_build(config: &mut Map<String, Value>) {
	if let Some(build_config) = config.get_mut("build").and_then(|b| b.as_object_mut()) {
		if let Some((dist_dir, key)) = build_config
			...
			.or_else(|| build_config.remove("dist-dir").map(|v| (v, "frontend-dist")))
		{
		...
		if let Some((with_global_tauri, key)) = build_config
			...
			.or_else(|| build_config.remove("with-global-tauri").map(|v| (v, "with-global-tauri")))
		{
			config
				.get_mut("tauri")
...
fn process_bundle(config: &mut Map<String, Value>, migrated: &MigratedConfig) {
		...
		.and_then(|t| t.remove("bundle"))
	{
		if let Some(bundle_config) = bundle_config.as_object_mut() {
			...
			// license file
			if let Some(macos) = bundle_config.get_mut("macOS").and_then(|v| v.as_object_mut()) {
...
fn process_allowlist(
	...
	if allowlist.protocol.asset_scope != Default::default() {
		...
		asset_protocol.insert("scope".into(), serde_json::to_value(allowlist.protocol.asset_scope.clone())?);
...
fn allowlist_to_permissions(allowlist: tauri_utils_v1::config::AllowlistConfig) -> Vec<PermissionEntry> {
	...
	if allowlist.all
		|| allowlist.shell.all
		|| !matches!(allowlist.shell.open, tauri_utils_v1::config::ShellAllowlistOpen::Flag(false))
	{
...
fn process_updater(
	...
	if let Some(mut updater) = tauri_config.remove("updater") {
		if let Some(updater) = updater.as_object_mut() {
			...
			if updater.remove("active").and_then(|a| a.as_bool()).unwrap_or_default() || updater.get("pubkey").is_some()
			{
...
mod test {
	...
	fn migrate_full() {
		...
		assert_eq!(migrated["plugins"]["updater"]["pubkey"], original["tauri"]["updater"]["pubkey"]);
		assert_eq!(
		...
		// security pattern
		assert_eq!(migrated["app"]["security"]["pattern"], original["tauri"]["pattern"]);
		...
		assert_eq!(migrated["bundle"]["linux"]["appimage"], original["tauri"]["bundle"]["appimage"]);

		// app information
		assert_eq!(migrated["productName"], original["package"]["productName"]);
		assert_eq!(migrated["mainBinaryName"], original["package"]["productName"]);
	...
	fn migrate_dangerous_use_http_scheme() {
		...
		let migrated = migrate(&original);
		assert_eq!(
			!migrated["app"]["windows"][0]["useHttpsScheme"].as_bool().unwrap(),
			original["tauri"]["security"]["dangerousUseHttpScheme"].as_bool().unwrap()
		);
	}
	...
	fn can_migrate_api_example_config() {
		let original = serde_json::from_str(include_str!("./fixtures/api-example.tauri.conf.json")).unwrap();
		migrate(&original);
	}

	#[test]
	fn can_migrate_cli_template_config() {
		let original = serde_json::from_str(include_str!("./fixtures/cli-template.tauri.conf.json")).unwrap();
		migrate(&original);
	}
	...
	fn migrate_csp_existing_connect_src_string() {
		...
		assert_eq!(
			...
			format!(
				...
				original["tauri"]["security"]["csp"]["connect-src"].as_str().unwrap()
			)
		);
	}
	...
	fn migrate_csp_existing_connect_src_array() {
		...
		let migrated_connect_src = migrated["app"]["security"]["csp"]["connect-src"]
			.as_array()
			.expect("connect-src isn't an array");
		let original_connect_src = original["tauri"]["security"]["csp"]["connect-src"].as_array().unwrap();
		assert!(
	...
	fn migrate_webview_fixed_runtime_path() {
		...
		assert_eq!(migrated["bundle"]["windows"]["webviewInstallMode"]["type"], "fixedRuntime");
