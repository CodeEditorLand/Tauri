...
pub fn migrate(frontend_dir: &Path) -> Result<Vec<String>> {
	...
	let npm_version = if pre.is_empty() {
		...
		format!("{}.0.0-{}.0", env!("CARGO_PKG_VERSION_MAJOR"), pre.split('.').next().unwrap())
	};
	...
	for entry in walk_builder(frontend_dir).build().flatten() {
		if entry.file_type().map(|t| t.is_file()).unwrap_or_default() {
			...
			if JS_EXTENSIONS.iter().any(|e| e == &ext) {
				...
				let new_contents = migrate_imports(path, &js_contents, &mut new_plugins, &mut npm_packages_to_remove)?;
...
fn migrate_imports<'a>(
	...
	let has_partial_js = path.extension().is_some_and(|ext| ext == "vue" || ext == "svelte");
...
"#;
		...
		let migrated =
			migrate_imports(Path::new("file.vue"), input, &mut new_plugins, &mut npm_packages_to_remove).unwrap();
...
"#;
		...
		let migrated =
			migrate_imports(Path::new("file.svelte"), input, &mut new_plugins, &mut npm_packages_to_remove).unwrap();
...
"#;
		...
		let migrated =
			migrate_imports(Path::new("file.js"), input, &mut new_plugins, &mut npm_packages_to_remove).unwrap();
