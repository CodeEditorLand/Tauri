...
pub fn command(options: Options) -> Result<()> {
	...
	let (config, metadata, cli_options) = {
		...
		let (config, metadata) = get_config(
			&get_app(MobileTarget::Android, tauri_config_, &AppInterface::new(tauri_config_, None)?),
			tauri_config_,
			None,
			&cli_options,
		);
	...
	if !cli_options.config.is_empty() {
		crate::helpers::config::merge_with(&cli_options.config.iter().map(|conf| &conf.0).collect::<Vec<_>>())?;
	}

	let env = env()?;

	if cli_options.dev {
		let dev_url = tauri_config.lock().unwrap().as_ref().unwrap().build.dev_url.clone();
	...
	call_for_targets_with_fallback(
		...
		|target: &Target| {
			...
			if !validated_lib {
				...
				let lib_path = config.app().target_dir(target.triple, profile).join(config.so_name());
...
fn validate_lib(path: &Path) -> Result<()> {
	...
	let elf = elf::ElfBytes::<elf::endian::AnyEndian>::minimal_parse(&so_bytes).context("failed to parse ELF")?;
...
fn adb_forward_port(port: u16, env: &cargo_mobile2::android::env::Env, cli_options: &CliOptions) -> Result<()> {
	...
	if devices.is_empty() {
		log::warn!("ADB device list is empty, waiting a few seconds to see if there's any booting device...");
		...
		for device in &devices {
			...
			if String::from_utf8_lossy(&reverse_list_output.stdout).contains(&forward) {
				...
				if device.serial_no() == target_device_serial_no {
					log::debug!("device {} already has the forward for {}", device.name(), forward);
		...
		if already_forwarded {
			...
			loop {
				...
				if String::from_utf8_lossy(&reverse_list_output.stdout).contains(&forward) {
					...
					log::warn!("waiting for the port to be forwarded to {}...", target_device_name);
					std::thread::sleep(std::time::Duration::from_secs(1));
				}
			}
		}
