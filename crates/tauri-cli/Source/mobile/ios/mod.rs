...
pub fn get_config(
	...
	for framework in tauri_config.bundle.ios.frameworks.clone().unwrap_or_default() {
		...
		if ext.is_empty() {
			...
			frameworks.push(framework_path.file_stem().unwrap().to_string_lossy().to_string());
		} else {
...
pub fn synchronize_project_config(
	...
	if let Some(build_configuration) = build_configuration {
		if let Some(style) = build_configuration.get_build_setting("CODE_SIGN_STYLE") {
			export_options_plist.insert("signingStyle".to_string(), style.value.to_lowercase().into());
		} else {
		...
		if manual_signing {
			...
			{
				export_options_plist.insert("signingCertificate".to_string(), identity.value.trim_matches('"').into());
			}

			let profile_uuid = project_config.provisioning_profile_uuid.clone().or_else(|| {
				build_configuration
			...
			if let Some(profile_uuid) = profile_uuid {
				...
				export_options_plist.insert("provisioningProfiles".to_string(), provisioning_profiles.into());
			}
		}
