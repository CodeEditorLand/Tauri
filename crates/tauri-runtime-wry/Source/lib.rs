...
//! compatibility in the future. The major version only signifies the intended
//! Tauri version.
...
#[cfg(target_os = "macos")]
pub use tao::platform::macos::{ActivationPolicy as TaoActivationPolicy, EventLoopExtMacOS, WindowExtMacOS};
#[cfg(target_os = "macos")]
...
pub use tao::{
	self,
	window::{Window, WindowBuilder as TaoWindowBuilder, WindowId as TaoWindowId},
};
use tao::{
	dpi::{
		LogicalPosition as TaoLogicalPosition,
		LogicalSize as TaoLogicalSize,
		PhysicalPosition as TaoPhysicalPosition,
		PhysicalSize as TaoPhysicalSize,
		Position as TaoPosition,
		Size as TaoSize,
	},
	event::{Event, StartCause, WindowEvent as TaoWindowEvent},
	event_loop::{
		ControlFlow,
		DeviceEventFilter as TaoDeviceEventFilter,
		EventLoop,
		EventLoopBuilder,
		EventLoopProxy as TaoEventLoopProxy,
		EventLoopWindowTarget,
	},
	monitor::MonitorHandle,
	window::{
		CursorIcon as TaoCursorIcon,
		Fullscreen,
...
impl WindowIdStore {
	pub fn insert(&self, w:TaoWindowId, id:WindowId) { self.0.lock().unwrap().insert(w, id); }

	fn get(&self, w:&TaoWindowId) -> Option<WindowId> { self.0.lock().unwrap().get(w).copied() }
}
...
impl<T:UserEvent> Context<T> {
	...
	fn next_window_id(&self) -> WindowId { self.next_window_id.fetch_add(1, Ordering::Relaxed).into() }

	fn next_webview_id(&self) -> WebviewId { self.next_webview_id.fetch_add(1, Ordering::Relaxed) }

	fn next_window_event_id(&self) -> u32 { self.next_window_event_id.fetch_add(1, Ordering::Relaxed) }

	fn next_webview_event_id(&self) -> u32 { self.next_webview_event_id.fetch_add(1, Ordering::Relaxed) }
}
...
impl<T:UserEvent> Context<T> {
	fn create_window<F:Fn(RawWindow) + Send + 'static>(
		...
		let (webview_id, use_https_scheme) = pending
			.webview
			.as_ref()
			.map(|w| (Some(context.next_webview_id()), w.webview_attributes.use_https_scheme))
		...
		let dispatcher = WryWindowDispatcher { window_id, context:self.clone() };
		...
		let detached_webview = webview_id.map(|id| {
			...
			DetachedWindowWebview { webview, use_https_scheme }
		});

		Ok(DetachedWindow { id:window_id, label, dispatcher, webview:detached_webview })
	}
	...
	fn create_webview(
		...
		let dispatcher = WryWebviewDispatcher { window_id:window_id_wrapper, webview_id, context:self.clone() };
...
pub enum ActiveTracingSpan {
	WindowDraw { id:TaoWindowId, span:tracing::span::EnteredSpan },
}
...
impl From<tauri_runtime::Rect> for RectWrapper {
	fn from(value:tauri_runtime::Rect) -> Self { RectWrapper(wry::Rect { position:value.position, size:value.size }) }
}
...
impl<'a> From<&TaoWindowEvent<'a>> for WindowEventWrapper {
	fn from(event:&TaoWindowEvent<'a>) -> Self {
		let event = match event {
			...
			TaoWindowEvent::Moved(position) => WindowEvent::Moved(PhysicalPositionWrapper(*position).into()),
...
impl<T> From<PhysicalPositionWrapper<T>> for PhysicalPosition<T> {
	fn from(position:PhysicalPositionWrapper<T>) -> Self { Self { x:position.0.x, y:position.0.y } }
}

impl<T> From<PhysicalPosition<T>> for PhysicalPositionWrapper<T> {
	fn from(position:PhysicalPosition<T>) -> Self { Self(TaoPhysicalPosition { x:position.x, y:position.y }) }
}

struct LogicalPositionWrapper<T>(TaoLogicalPosition<T>);

impl<T> From<LogicalPosition<T>> for LogicalPositionWrapper<T> {
	fn from(position:LogicalPosition<T>) -> Self { Self(TaoLogicalPosition { x:position.x, y:position.y }) }
}

pub struct PhysicalSizeWrapper<T>(pub TaoPhysicalSize<T>);

impl<T> From<PhysicalSizeWrapper<T>> for PhysicalSize<T> {
	fn from(size:PhysicalSizeWrapper<T>) -> Self { Self { width:size.0.width, height:size.0.height } }
}

impl<T> From<PhysicalSize<T>> for PhysicalSizeWrapper<T> {
	fn from(size:PhysicalSize<T>) -> Self { Self(TaoPhysicalSize { width:size.width, height:size.height }) }
}

struct LogicalSizeWrapper<T>(TaoLogicalSize<T>);

impl<T> From<LogicalSize<T>> for LogicalSizeWrapper<T> {
	fn from(size:LogicalSize<T>) -> Self { Self(TaoLogicalSize { width:size.width, height:size.height }) }
}
...
impl WindowBuilder for WindowBuilderWrapper {
	fn new() -> Self {
		...
		#[cfg(target_os = "macos")]
		{
			...
			// By setting the default style to `TitleBarStyle::Visible` should fix the issue
	...
	fn with_config(config:&WindowConfig) -> Self {
		...
		#[cfg(target_os = "macos")]
		{
			...
			if let Some(position) = &config.traffic_light_position {
				window =
					window.traffic_light_position(tauri_runtime::dpi::LogicalPosition::new(position.x, position.y));
			}
		}
        ...
        This can be enabled \
				 via the `tauri.macOSPrivateApi` configuration property <https://v2.tauri.app/reference/config/#macosprivateapi>\
				 "
			);
		}
		...
		#[cfg(target_os = "linux")]
		{
			// Mouse event is disabled on Linux since sudden event bursts could block event
			// loop.
	...
	fn inner_size(mut self, width:f64, height:f64) -> Self {
		self.inner = self.inner.with_inner_size(TaoLogicalSize::new(width, height));
		self
	}

	fn min_inner_size(mut self, min_width:f64, min_height:f64) -> Self {
		self.inner = self.inner.with_min_inner_size(TaoLogicalSize::new(min_width, min_height));
		self
	}

	fn max_inner_size(mut self, max_width:f64, max_height:f64) -> Self {
		self.inner = self.inner.with_max_inner_size(TaoLogicalSize::new(max_width, max_height));
		self
	}
	...
	fn skip_taskbar(self, _skip:bool) -> Self { self }
	...
	fn has_icon(&self) -> bool { self.inner.window.window_icon.is_some() }

	fn get_theme(&self) -> Option<Theme> {
		self.inner.window.preferred_theme.map(|theme| {
			match theme {
				TaoTheme::Dark => Theme::Dark,
				_ => Theme::Light,
			}
	...
	#[cfg(not(windows))]
	fn window_classname<S:Into<String>>(self, _window_classname:S) -> Self { self }
}
...
impl<T:UserEvent> WebviewDispatch<T> for WryWebviewDispatcher<T> {
	...
	fn open_devtools(&self) {
		let _ = send_user_message(
			&self.context,
			Message::Webview(*self.window_id.lock().unwrap(), self.webview_id, WebviewMessage::OpenDevTools),
		);
	}
	...
	fn close_devtools(&self) {
		let _ = send_user_message(
			&self.context,
			Message::Webview(*self.window_id.lock().unwrap(), self.webview_id, WebviewMessage::CloseDevTools),
		);
	}

	/// Gets the devtools window's current open state.
	#[cfg(any(debug_assertions, feature = "devtools"))]
	fn is_devtools_open(&self) -> Result<bool> { webview_getter!(self, WebviewMessage::IsDevToolsOpen) }

	// Getters

	fn url(&self) -> Result<String> { webview_getter!(self, WebviewMessage::Url)? }

	fn bounds(&self) -> Result<tauri_runtime::Rect> { webview_getter!(self, WebviewMessage::Bounds)? }

	fn position(&self) -> Result<PhysicalPosition<i32>> { webview_getter!(self, WebviewMessage::Position)? }

	fn size(&self) -> Result<PhysicalSize<u32>> { webview_getter!(self, WebviewMessage::Size)? }
	...
	fn navigate(&self, url:Url) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Webview(*self.window_id.lock().unwrap(), self.webview_id, WebviewMessage::Navigate(url)),
		)
	}
	...
	fn reload(&self) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Webview(*self.window_id.lock().unwrap(), self.webview_id, WebviewMessage::Reload),
		)
	}
	...
	fn print(&self) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Webview(*self.window_id.lock().unwrap(), self.webview_id, WebviewMessage::Print),
		)
	}
	...
	fn close(&self) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Webview(*self.window_id.lock().unwrap(), self.webview_id, WebviewMessage::Close),
		)
	}
	...
	fn set_size(&self, size:Size) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Webview(*self.window_id.lock().unwrap(), self.webview_id, WebviewMessage::SetSize(size)),
		)
	}
	...
	fn set_focus(&self) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Webview(*self.window_id.lock().unwrap(), self.webview_id, WebviewMessage::SetFocus),
		)
	}
	...
	fn reparent(&self, window_id:WindowId) -> Result<()> {
		...
		send_user_message(
			&self.context,
			Message::Webview(*current_window_id, self.webview_id, WebviewMessage::Reparent(window_id, tx)),
		)?;
	...
	fn cookies_for_url(&self, url:Url) -> Result<Vec<Cookie<'static>>> {
		...
		send_user_message(
			&self.context,
			Message::Webview(*current_window_id, self.webview_id, WebviewMessage::CookiesForUrl(url, tx)),
		)?;

		rx.recv().unwrap()
	}

	fn cookies(&self) -> Result<Vec<Cookie<'static>>> { webview_getter!(self, WebviewMessage::Cookies)? }
	...
	fn hide(&self) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Webview(*self.window_id.lock().unwrap(), self.webview_id, WebviewMessage::Hide),
		)
	}
	...
	fn show(&self) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Webview(*self.window_id.lock().unwrap(), self.webview_id, WebviewMessage::Show),
		)
	}
...
impl<T:UserEvent> WindowDispatch<T> for WryWindowDispatcher<T> {
	...
	fn scale_factor(&self) -> Result<f64> { window_getter!(self, WindowMessage::ScaleFactor) }

	fn inner_position(&self) -> Result<PhysicalPosition<i32>> { window_getter!(self, WindowMessage::InnerPosition)? }

	fn outer_position(&self) -> Result<PhysicalPosition<i32>> { window_getter!(self, WindowMessage::OuterPosition)? }

	fn inner_size(&self) -> Result<PhysicalSize<u32>> { window_getter!(self, WindowMessage::InnerSize) }

	fn outer_size(&self) -> Result<PhysicalSize<u32>> { window_getter!(self, WindowMessage::OuterSize) }

	fn is_fullscreen(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsFullscreen) }

	fn is_minimized(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsMinimized) }

	fn is_maximized(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsMaximized) }

	fn is_focused(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsFocused) }

	/// Gets the window's current decoration state.
	fn is_decorated(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsDecorated) }

	/// Gets the window's current resizable state.
	fn is_resizable(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsResizable) }

	/// Gets the current native window's maximize button state
	fn is_maximizable(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsMaximizable) }

	/// Gets the current native window's minimize button state
	fn is_minimizable(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsMinimizable) }

	/// Gets the current native window's close button state
	fn is_closable(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsClosable) }

	fn is_visible(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsVisible) }

	fn title(&self) -> Result<String> { window_getter!(self, WindowMessage::Title) }
	...
	fn theme(&self) -> Result<Theme> { window_getter!(self, WindowMessage::Theme) }

	fn is_enabled(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsEnabled) }

	fn is_always_on_top(&self) -> Result<bool> { window_getter!(self, WindowMessage::IsAlwaysOnTop) }
	...
	fn default_vbox(&self) -> Result<gtk::Box> { window_getter!(self, WindowMessage::GtkBox).map(|w| w.0) }
	...
	fn destroy(&self) -> Result<()> {
		// NOTE: destroy cannot use the `send_user_message` function because it accesses
		// the event loop callback
		self.context
			.proxy
	...
	fn set_shadow(&self, enable:bool) -> Result<()> {
		send_user_message(&self.context, Message::Window(self.window_id, WindowMessage::SetShadow(enable)))
	}
	...
	fn set_content_protected(&self, protected:bool) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Window(self.window_id, WindowMessage::SetContentProtected(protected)),
		)
	}
	...
	fn set_size_constraints(&self, constraints:WindowSizeConstraints) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Window(self.window_id, WindowMessage::SetSizeConstraints(constraints)),
		)
	}
	...
	fn set_icon(&self, icon:Icon) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Window(self.window_id, WindowMessage::SetIcon(TaoIcon::try_from(icon)?.0)),
		)
	}
	...
	fn set_cursor_position<Pos:Into<Position>>(&self, position:Pos) -> crate::Result<()> {
		send_user_message(
			&self.context,
			Message::Window(self.window_id, WindowMessage::SetCursorPosition(position.into())),
		)
	}
	...
	fn set_badge_count(&self, count:Option<i64>, desktop_filename:Option<String>) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Window(self.window_id, WindowMessage::SetBadgeCount(count, desktop_filename)),
		)
	}
	...
	fn set_progress_bar(&self, progress_state:ProgressBarState) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Window(self.window_id, WindowMessage::SetProgressBar(progress_state)),
		)
	}
	...
	fn set_traffic_light_position(&self, position:Position) -> Result<()> {
		send_user_message(
			&self.context,
			Message::Window(self.window_id, WindowMessage::SetTrafficLightPosition(position)),
		)
	}

	fn set_theme(&self, theme:Option<Theme>) -> Result<()> {
		send_user_message(&self.context, Message::Window(self.window_id, WindowMessage::SetTheme(theme)))
	}
...
impl<T:UserEvent> WryHandle<T> {
	...
	pub fn window_id(&self, window_id:TaoWindowId) -> WindowId {
		*self.context.window_id_map.0.lock().unwrap().get(&window_id).unwrap()
	}
...
impl<T:UserEvent> RuntimeHandle<T> for WryHandle<T> {
	...
	fn create_proxy(&self) -> EventProxy<T> { EventProxy(self.context.proxy.clone()) }

	#[cfg(target_os = "macos")]
	fn set_activation_policy(&self, activation_policy:ActivationPolicy) -> Result<()> {
		send_user_message(&self.context, Message::SetActivationPolicy(activation_policy))
	}

	fn request_exit(&self, code:i32) -> Result<()> {
		// NOTE: request_exit cannot use the `send_user_message` function because it
		// accesses the event loop callback
		self.context
			.proxy
	...
	fn set_theme(&self, theme:Option<Theme>) {
		self.context.main_thread.window_target.set_theme(match theme {
...
impl<T:UserEvent> Runtime<T> for Wry<T> {
	...
	fn create_proxy(&self) -> EventProxy<T> { EventProxy(self.event_loop.create_proxy()) }

	fn handle(&self) -> Self::Handle { WryHandle { context:self.context.clone() } }
	...
	fn create_window<F:Fn(RawWindow) + Send + 'static>(
		...
		let (webview_id, use_https_scheme) = pending
			.webview
			.as_ref()
			.map(|w| (Some(self.context.next_webview_id()), w.webview_attributes.use_https_scheme))
			.unwrap_or((None, false));
		...
		let dispatcher = WryWindowDispatcher { window_id, context:self.context.clone() };

		self.context.main_thread.windows.0.borrow_mut().insert(window_id, window);
	...
	fn create_webview(&self, window_id:WindowId, pending:PendingWebview<T, Self>) -> Result<DetachedWebview<T, Self>> {
		...
		if let Some(window) = window {
			...
			self.context.main_thread.windows.0.borrow_mut().get_mut(&window_id).map(|w| {
			...
			let dispatcher =
				WryWebviewDispatcher { window_id:window_id_wrapper, webview_id, context:self.context.clone() };
	...
	fn set_activation_policy(&mut self, activation_policy:ActivationPolicy) {
		self.event_loop.set_activation_policy(tao_activation_policy(activation_policy));
	}

	#[cfg(target_os = "macos")]
	fn show(&self) { self.event_loop.show_application(); }

	#[cfg(target_os = "macos")]
	fn hide(&self) { self.event_loop.hide_application(); }
...
fn handle_user_message<T:UserEvent>(
	...
	let UserMessageContext { window_id_map, windows } = context;
	match message {
		...
		Message::Window(id, window_message) => {
			...
			if let Some((Some(window), webviews, has_children, window_event_listeners)) = w {
				match window_message {
					...
					WindowMessage::MonitorFromPoint(tx, (x, y)) => tx.send(window.monitor_from_point(x, y)).unwrap(),
					...
					WindowMessage::GtkBox(tx) => tx.send(GtkBox(window.default_vbox().unwrap().clone())).unwrap(),
					WindowMessage::RawWindowHandle(tx) => {
						tx.send(window.window_handle().map(|h| SendRawWindowHandle(h.as_raw())))
							.unwrap()
					},
					...
					WindowMessage::SetBackgroundColor(color) => window.set_background_color(color.map(Into::into)),
				}
			}
		},
		Message::Webview(window_id, webview_id, webview_message) => {
			...
			let webview_handle = windows
				...
				.borrow()
				.get(&window_id)
				.map(|w| (w.inner.clone(), w.webviews.iter().find(|w| w.id == webview_id).cloned()));
			if let Some((Some(window), Some(webview))) = webview_handle {
				match webview_message {
					...
					WebviewMessage::AddEventListener(id, listener) => {
						webview.webview_event_listeners.lock().unwrap().insert(id, listener);
					},
					...
					WebviewMessage::Cookies(tx) => {
						tx.send(webview.cookies().map_err(|_| Error::FailedToSendMessage)).unwrap();
					},

					WebviewMessage::CookiesForUrl(url, tx) => {
						let webview_cookies =
							webview.cookies_for_url(url.as_str()).map_err(|_| Error::FailedToSendMessage);
					...
					WebviewMessage::Bounds(tx) => {
						tx.send(
							webview
								.bounds()
								.map(|bounds| tauri_runtime::Rect { size:bounds.size, position:bounds.position })
								.map_err(|_| Error::FailedToSendMessage),
						)
						.unwrap();
					},
					...
					WebviewMessage::WithWebview(f) => {
						...
						#[cfg(target_os = "ios")]
						{
							...
							f(Webview {
								webview:Retained::into_raw(webview.inner.webview()) as *mut objc2::runtime::AnyObject
									as *mut std::ffi::c_void,
								manager:Retained::into_raw(webview.inner.manager()) as *mut objc2::runtime::AnyObject
									as *mut std::ffi::c_void,
						...
						#[cfg(windows)]
						{
							f(Webview { controller:webview.controller() });
						}
						#[cfg(target_os = "android")]
						{
							f(webview.handle())
		...
		Message::CreateWebview(window_id, handler) => {
			let window = windows.0.borrow().get(&window_id).and_then(|w| w.inner.clone());
			if let Some(window) = window {
		...
		Message::CreateWindow(window_id, handler) => {
		...
		Message::UserEvent(_) => (),
		Message::EventLoopWindowTarget(message) => {
			match message {
				EventLoopWindowTargetMessage::CursorPosition(sender) => {
					let pos = event_loop.cursor_position().map_err(|_| Error::FailedToSendMessage);
...
fn handle_event_loop<T:UserEvent>(
	...
	match event {
		...
		Event::UserEvent(Message::Webview(window_id, webview_id, WebviewMessage::WebviewEvent(event))) => {
		...
		Event::UserEvent(Message::Webview(window_id, _webview_id, WebviewMessage::SynthesizedWindowEvent(event))) => {
		...
		Event::UserEvent(message) => {
			match message {
				Message::RequestExit(code) => {
					let (tx, rx) = channel();
					callback(RunEvent::ExitRequested { code:Some(code), tx });
				...
				message => {
					handle_user_message(event_loop, message, UserMessageContext { window_id_map, windows });
				},
			}
		},
		#[cfg(any(target_os = "macos", target_os = "ios"))]
		...
		#[cfg(target_os = "macos")]
		Event::Reopen { has_visible_windows, .. } => callback(RunEvent::Reopen { has_visible_windows }),
		_ => (),
	}
}
...
fn on_close_requested<'a, T:UserEvent>(
	...
	if let Some(w) = windows_ref.get(&window_id) {
		...
		callback(RunEvent::WindowEvent { label, event:WindowEvent::CloseRequested { signal_tx:tx } });
...
fn create_window<T:UserEvent, F:Fn(RawWindow) + Send + 'static>(
	...
	#[allow(unused_mut)]
	let PendingWindow { mut window_builder, label, webview } = pending;
	...
	#[cfg(feature = "tracing")]
	let window_create_span = tracing::debug_span!(parent: &window_draw_span, "wry::window::create").entered();
	...
	#[cfg(feature = "tracing")]
	{
		...
		context
			...
			.0
			.borrow_mut()
			.push(ActiveTracingSpan::WindowDraw { id:window.id(), span:window_draw_span });
	}
...
fn create_webview<T:UserEvent>(
	...
	#[allow(unused_mut)]
	let PendingWebview { webview_attributes, uri_scheme_protocols, label, ipc_handler, url, .. } = pending;

	let mut web_context = context.main_thread.web_context.lock().expect("poisoned WebContext store");
	let is_first_context = web_context.is_empty();
	// the context must be stored on the HashMap because it must outlive the WebView
	// on macOS
	...
	if let Some(background_throttling) = webview_attributes.background_throttling {
		webview_builder = webview_builder.with_background_throttling(match background_throttling {
			tauri_utils::config::BackgroundThrottlingPolicy::Disabled => wry::BackgroundThrottlingPolicy::Disabled,
			tauri_utils::config::BackgroundThrottlingPolicy::Suspend => wry::BackgroundThrottlingPolicy::Suspend,
			tauri_utils::config::BackgroundThrottlingPolicy::Throttle => wry::BackgroundThrottlingPolicy::Throttle,
		});
	}
	...
	if webview_attributes.drag_drop_handler_enabled {
		...
		webview_builder = webview_builder.with_drag_drop_handler(move |event| {
			let event = match event {
				WryDragDropEvent::Enter { paths, position: (x, y) } => {
					DragDropEvent::Enter { paths, position:PhysicalPosition::new(x as _, y as _) }
				},
				WryDragDropEvent::Over { position: (x, y) } => {
					DragDropEvent::Over { position:PhysicalPosition::new(x as _, y as _) }
				},
				WryDragDropEvent::Drop { paths, position: (x, y) } => {
					DragDropEvent::Drop { paths, position:PhysicalPosition::new(x as _, y as _) }
				},
	...
	if let Some(navigation_handler) = pending.navigation_handler {
		webview_builder = webview_builder
			.with_navigation_handler(move |url| url.parse().map(|url| navigation_handler(&url)).unwrap_or(true));
	}
	...
	let webview_bounds = if let Some(bounds) = webview_attributes.bounds {
		...
		{
			webview_builder = webview_builder
				.with_bounds(wry::Rect { position:LogicalPosition::new(0, 0).into(), size:window.inner_size().into() });
			Some(WebviewBounds { x_rate:0., y_rate:0., width_rate:1., height_rate:1. })
		}
		#[cfg(not(feature = "unstable"))]
		None
	};
	...
	if let Some(download_handler) = pending.download_handler {
		...
		webview_builder = webview_builder.with_download_started_handler(move |url, path| {
			if let Ok(url) = url.parse() {
				download_handler_(DownloadEvent::Requested { url, destination:path })
			} else {
				false
			}
		});
	...
	{
		webview_builder =
			webview_builder.with_browser_extensions_enabled(webview_attributes.browser_extensions_enabled);
	}
	...
	for (scheme, protocol) in uri_scheme_protocols {
		...
		))]
		{
			...
			web_context.registered_custom_protocols.insert(scheme.clone());
		}

		webview_builder =
			webview_builder.with_asynchronous_custom_protocol(scheme, move |webview_id, request, responder| {
				protocol(webview_id, request, Box::new(move |response| responder.respond(response)))
			});
	}
...
#[cfg(target_os = "macos")]
fn inner_size(window:&Window, webviews:&[WebviewWrapper], has_children:bool) -> TaoPhysicalSize<u32> {
