...
mod windows {
	...
	pub fn attach_resize_handler(hwnd:isize, has_undecorated_shadows:bool) {
		...
		let data = UndecoratedResizingData { child:HWND::default(), has_undecorated_shadows };
		...
		unsafe {
			...
			let data = UndecoratedResizingData { child:drag_window, has_undecorated_shadows };
	...
	#[allow(non_snake_case)]
	#[inline]
	fn GET_Y_LPARAM(lparam:LPARAM) -> i16 { (((lparam.0 as usize) & 0xFFFF_0000) >> 16) as u16 as i16 }
}
