...
fn main() {
	...
	if target_os == "android" {
		...
		if let Ok(kotlin_out_dir) = std::env::var("WRY_ANDROID_KOTLIN_FILES_OUT_DIR") {
			...
			let kotlin_files_path = PathBuf::from(env_var("CARGO_MANIFEST_DIR")).join("mobile/android-codegen");
			println!("cargo:rerun-if-changed={}", kotlin_files_path.display());
			let kotlin_files = fs::read_dir(kotlin_files_path).expect("failed to read Android codegen directory");
		...
		let lib_path = PathBuf::from(std::env::var_os("CARGO_MANIFEST_DIR").unwrap()).join("mobile/android");
	...
	{
		if target_os == "ios" {
			let lib_path = PathBuf::from(std::env::var_os("CARGO_MANIFEST_DIR").unwrap()).join("mobile/ios-api");
	...
	// but we need to do this here for the examples in this workspace to work as
	// they don't have build scripts
...
fn define_permissions(out_dir:&Path) -> BTreeMap<String, Vec<tauri_utils::acl::manifest::PermissionFile>> {
	...
	for (plugin, commands) in PLUGINS {
		...
		let autogenerated = permissions_out_dir.join(tauri_utils::acl::build::AUTOGENERATED_FOLDER_NAME);
...
"###,
		...
		let permissions = tauri_utils::acl::build::define_permissions(
			&PathBuf::from(glob::Pattern::escape(&permissions_out_dir.to_string_lossy()))
				.join("**")
				.join("*.toml")
				.to_string_lossy(),
			&format!("tauri:{plugin}"),
		...
		let docs_out_dir = Path::new("permissions").join(plugin_directory_name).join("autogenerated");
...
fn define_default_permission_set(out_dir:&Path) -> Vec<tauri_utils::acl::manifest::PermissionFile> {
	let permissions_out_dir = out_dir.join("permissions");
	fs::create_dir_all(&permissions_out_dir).expect("failed to create core:default permissions directory");
...
"#,
	...
	write_if_changed(default_toml, toml_content).unwrap_or_else(|_| panic!("unable to autogenerate core:default set"));
