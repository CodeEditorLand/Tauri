...
pub fn notarize_auth() -> Result<tauri_macos_sign::AppleNotarizationCredentials, NotarizeAuthError> {
	match (var_os("APPLE_ID"), var_os("APPLE_PASSWORD"), var_os("APPLE_TEAM_ID")) {
		(Some(apple_id), Some(password), Some(team_id)) => {
			Ok(tauri_macos_sign::AppleNotarizationCredentials::AppleId { apple_id, password, team_id })
		},
		(Some(_apple_id), Some(_password), None) => Err(NotarizeAuthError::MissingTeamId),
		_ => match (var_os("APPLE_API_KEY"), var_os("APPLE_API_ISSUER"), var("APPLE_API_KEY_PATH")) {
			(Some(key_id), Some(issuer), Ok(key_path)) => Ok(tauri_macos_sign::AppleNotarizationCredentials::ApiKey {
				key_id,
