...
pub fn command(options: Options) -> Result<()> {
	...
	let permissions: HashSet<String> = match options.permission.map(FromIterator::from_iter) {
		Some(p) => p,
		None => prompts::input::<String>("What permissions to enable? (comma separated)", None, false, true)?
			.and_then(|p| {
				if p.is_empty() {
					None
				} else {
		...
		permissions: permissions
			...
			.map(|p| {
				PermissionEntry::PermissionRef(
					p.clone().try_into().unwrap_or_else(|_| panic!("invalid permission {}", p)),
				)
			})
			.collect(),
		platforms: None,
	...
	let path = match options.out {
		...
		None => {
			...
			capabilities_dir.join(format!("{}.{}", capability.identifier, options.format.extension()))
		},
	};
